...
namespace MesWorklog.Controllers
{
    ...
    public class LineController : ControllerBase
    {
        ...
        // 생성자 주입. DI 컨테이너가 요청마다 LineService를 만들어 넣어준다
        ...
        // HTTP GET 프로토콜로 요청시 
        // GET /api/lines?includeInactive=false
        [HttpGet]
        public async Task<ActionResult<List<LineResponse>>> GetAll(
            // includeInactive: 비활성 라인까지 포함할지 여부
            // 기본값 false — 호출하는 쪽은 활성 라인만 받게
            // true - 비활성화 라인도 호출
            // 현재는 작업이력이 있어 비활성화 되었을 경우 다시 복구하는 웹 flow가 없음 
            bool includeInactive = false)
        {
            var lines = await _lineService.GetAllAsync(includeInactive);
        ...
        [HttpGet("{id:int}")]
        public async Task<ActionResult<LineResponse>> GetById(
            // id: 조회할 라인의 PK. URL 경로에서 값을 꺼낸다
            int id)
        {
            // 못찾으면 미들웨어에서 404 ProblemDetails로 변환한다 → 여기서 null 체크 불필요
            return Ok(await _lineService.GetByIdAsync(id));
        }

        // POST /api/lines
        [HttpPost]
        public async Task<ActionResult<LineResponse>> Create(
            // request: 생성할 라인 정보, 복합타입 이라 [FromBody]
            CreateLineRequest request)
        {
            var created = await _lineService.CreateAsync(request);
        ...
        // PUT /api/lines/?
        [HttpPut("{id:int}")]
        public async Task<ActionResult<LineResponse>> Update(
            // id: 수정 대상 라인의 PK (URL 경로)
            int id,
            // request: 새 이름(Name)과 활성 여부(IsActive)
            UpdateLineRequest request)
        {
            return Ok(await _lineService.UpdateAsync(id, request));
        }
