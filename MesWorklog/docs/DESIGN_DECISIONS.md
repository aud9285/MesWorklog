...
3. **상세조회 화면**(`DetailView.vue`): **날짜별 작업이력 목록(카드) → 선택 → 상세**(§4-15-⑤, ID 직접 입력 아님). 완료 건만 시간 분해(조업/가동/실가동 누적 막대 + 계산식)와 가동률 표시, 진행중/정지중 건은 경과 시간만. **라인/공정/설비 수정 다이얼로그** 제공 — 등록 화면과 동일한 캐스케이딩 규칙 적용(§4-9). 같은 작업지시에 형제 이력이 있으면 저장 전 확인 팝업(§4-15-③)
...
| 수정/저장(`PATCH` 등) | 원칙 미적용, **예외 1건** | 되돌릴 수 있는 작업이라 기본은 미적용. 단 작업지시 수정(§4-9)은 형제 `WorkLog`가 1건 이상이면 적용 — 본인 것만 고친다는 오해를 막기 위함(§4-15-③) |
