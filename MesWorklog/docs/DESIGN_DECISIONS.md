...
- `resumedAt > pausedAt`(정지 자신의 길이가 0/음수가 되는 것만 막음). `pausedAt >= 직전 시작/재개 시각`, `endTime >= 마지막 기록 시각` — **이전만 거부, 같음은 허용**(§4-20)
...
### 4-20. 정지/완료 시각 검증 — `<=` → `<`로 완화(정상 시나리오가 막히던 버그)

`Pause()`와 `Complete()`가 각각 "새 정지 시각은 직전 재개보다 **이후**(`pausedAt <= lastResumedAt`이면 거부)", "종료 시각은 마지막 기록 시각보다 **이후**(`endTime <= lastRecordedAt`이면 거부)"로 **같음도 막고 있었다.** 실사용 중 다음 두 시나리오가 그대로 걸린다는 걸 발견:

```
12:00~13:00 식사(계획정지) → 13:00 재개 → 그 자리에서 곧바로 13:00 설비고장(비가동) 정지
10:00~10:20 설비고장 → 10:20 재개(수리 완료) → 그 자리에서 곧바로 10:20 종료(퇴근)
```

둘 다 "사유가 바뀌며 곧바로 이어지는 정지"·"재개하자마자 마감"이라는 지극히 정상적인 현장 상황인데, 재개와 다음 기록이 정확히 같은 시각(10분 단위 입력이라 실제로 자주 발생)이면 400으로 거부됐다.

**원인은 서로 다른 두 불변식을 하나의 조건으로 뭉뚱그린 것.** `Resume()`의 `resumedAt <= pausedAt` 거부는 **정지 1건 자신의 길이**가 0/음수가 되는 걸 막는 것으로 여전히 유효하다. 반면 `Pause()`/`Complete()`가 막던 건 **직전 정지와 다음 정지(또는 종료) 사이의 가동시간이 0분**인 경우인데, 이건 막을 이유가 없다 — `NetOperatingMinutes` 계산(§3-4, "가동시간 = 부하시간 − 비가동시간")은 정지 구간들의 합을 빼는 구조라 그 사이 간격이 0분이어도 그냥 0만큼 덜 빠질 뿐 깨지지 않는다.

**대응**: `Pause()`의 `pausedAt <= lastResumedAt`을 `pausedAt < lastResumedAt`으로, `Complete()`의 `endTime <= lastRecordedAt`을 `endTime < lastRecordedAt`으로 완화 — "이전"만 거부하고 "같음"은 허용. `Resume()`의 `resumedAt <= pausedAt`는 손대지 않았다(§3-3 갱신).
...
- **`WorkLog` 상태 전이 xUnit 단위테스트** (2026-08-18 완료 — `MesWorklog.Tests/WorkLogTests.cs`, 13개)
  `Start`/`Pause`/`Resume`/`Complete`의 상태 가드와 시간 검증, 그리고 §3-4 시간 분해 계산(계획정지/비가동 차감)까지 커버한다. **§4-20에서 완화한 "같음 허용" 두 케이스를 회귀 테스트로 명시적으로 박아뒀다** — 이 두 개가 없으면 `<`를 실수로 `<=`로 되돌려도 어떤 테스트도 실패하지 않는다(기존 테스트는 전부 "이전 시각 → 거부"만 검증하고 있어서 경계값이 비어 있었음).
  이 테스트가 DB 없이 가능한 건 §3-2에서 `now`를 파라미터로 주입하도록 설계해둔 덕분이다 — 시계를 고정할 수 있어 "재개와 정지가 정확히 같은 시각" 같은 경계값을 즉시·반복 재현할 수 있다. 실제로 같은 경계를 브라우저로 확인하려 했을 때는 실제 시계가 10분 단위 경계를 넘길 때까지 기다려야 했다.
